...
def convert_file_from_windows_to_unix_eol(filename):
    """
    Convert a file from Windows EOL (CRLF) to Unix EOL (LF).
    
    Args:
        filename (str): The path to the file to convert.
    ...
    with open(filename, 'wb') as f:
        f.write(content)
    logging.info("Converted %s from Windows EOL to Unix EOL.", filename)
...
def convert_excel_to_yaml(filename, config_dict):
    ...
    # Description D5
    description = df_mapping.iloc[3, 3] if len(df_mapping) > 4 else None
    if not description:
        logging.warning(
            "Could not extract description from mapping sheet %s in %s. Using default value 'No Description'.",
            mapping_sheet_name,
            filename,
        )
        description = 'No Description'
    # Stream Name D6
    stream_name = df_mapping.iloc[4, 3] if len(df_mapping) > 5 else None
    if not stream_name:
        logging.warning(
            "Could not extract stream name from mapping sheet %s in %s. Using default value 'No Stream Name'.",
            mapping_sheet_name,
            filename,
        )
        stream_name = 'DEFAULT'
    # if stream_name is not a string then convert to string
    if not isinstance(stream_name, str):
        stream_name = str(stream_name)
    stream_name = stream_name.strip()
    # if stream_name is empty then set to 'DEFAULT'
    if not stream_name:
        stream_name = 'DEFAULT'
    # if stream_name is not a valid string then set to 'DEFAULT'
    if not re.match(r'^[a-zA-Z0-9_]+$', stream_name):
        logging.warning(
            "Stream name %s in mapping sheet %s in %s is not a valid string. "
    ...
    try:
        ...
        yaml_dict['job_name'] = job_name_from_file
        yaml_dict['description'] = description
        yaml_dict['stream_id']=stream_name
...
def build_job(filename, config_dict):
    ...
    sample_path = config_local.get('sample_path', 'sample_path')
    if not os.path.isdir(sample_path):
        logging.error(
            "Sample path %s does not exist. Please check your configuration.",
            sample_path,
        )
    ...
    if not os.path.isdir(sample_job_path):
        ...
        logging.info(
            "Sample job path %s does not exist. Creating it.",
            sample_job_path,
        )
    sample_job_file_path = os.path.join(sample_job_path, filename)
...
, config=config_local)
    ...
    try:
        ...
        # now do eol conversion to unix style
        convert_file_from_windows_to_unix_eol(job_file_path)
        logging.info("Created job file %s successfully.", job_file_path)
        # now copy the job file to sample job path
        sample_job_file_path = os.path.join(sample_job_path, job_file_name)
        shutil.copyfile(job_file_path, sample_job_file_path)
        logging.info("Copied job file %s to %s successfully.", job_file_path, sample_job_file_path)
    except (OSError, yaml.YAMLError) as e:
        logging.error(
    ...
    for step in yaml_dict.get('steps', []):
        ...
        # Now get sample path for the step file
        sample_path = config_local.get('sample_path', 'sample')
        # append steps to file path
        if not os.path.isdir(sample_path):
            logging.error(
                "Sample path %s does not exist. Please check your configuration.",
                sample_path,
        ...
        sample_step_path = os.path.join(sample_path, 'steps')
        if not os.path.isdir(sample_step_path):
            # mkdir the sample step path
            try:
                os.makedirs(sample_step_path)
            except OSError as e:
                logging.error("Error creating sample   step directory %s: %s",
                    sample_step_path,
                    str(e),
                )
                return -1, str(e)
        # now step file path
        sample_step_file_path = os.path.join(sample_step_path, step['file_name'])
        # Write the step to a file
        try:
            with open(step_file_path, 'w', encoding='utf-8') as step_file:
                step_file.write(step_rendered)
            # now do eol conversion to unix style
            convert_file_from_windows_to_unix_eol(step_file_path)
            # now copy file to sample step path
            shutil.copyfile(step_file_path, sample_step_file_path)
            logging.info("Created step file %s successfully.", step_file_path)
        except (OSError, yaml.YAMLError) as e:
            logging.error(
