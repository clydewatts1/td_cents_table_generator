...
/*----------------------------------------------------------------------
* FileName: T_ACC_FND.DW_FND_LOC_DIM.ddl
...
CREATE MULTISET TABLE DW${INSTANCE}T_ACC_FND.DW_FND_LOC_DIM
    ...
    location_type VARCHAR(3) NOT NULL   COMPRESS(FA','S','W')  ,
    currency_cd CHAR(3)    COMPRESS(GBP','EUR','USD')  ,
    ...
    vat_include_ind CHAR(1) NOT NULL   COMPRESS(Y')  ,
    store_open_dt DATE    ,
    store_close_dt DATE    ,
    start_order_days SMALLINT    ,
    selfridge_ind CHAR(1)    COMPRESS(N')  ,
    finisher_ind VARCHAR(10)    ,
    transfer_entity VARCHAR(50)    ,
    physical_warehouse_cd INTEGER    ,
