...
# ===============================================================================
# ===============================================================================
# File Name           : FND2130.300.TARGET.sql
# Pattern             : 
...
/* Set query band */
SET QUERY_BAND = 'ApplicationName=TJC;Group=FND_BASE;JobId=FND2130.300.TARGET.sql;JobSeq=300;Instance=T04;RUNID=4613;STREAMID=TEST;' UPDATE FOR SESSION
;
...
LOCKING TABLE DWT04V_ACC_FND.DW_FND_AGG_DAILY_STOCK_FCT FOR WRITE;
--.IF ERRORCODE <> 0 THEN .QUIT 101;
-- Before inserting new data, delete any existing records for the entire business week
...
DELETE FROM DWT04V_ACC_FND.DW_FND_AGG_DAILY_STOCK_FCT
WHERE
    business_date BETWEEN date '2025-08-10' AND date '2025-08-16'
;
--.IF ERRORCODE <> 0 THEN .QUIT 101;
-- Insert the aggregated data into the final fact table.
INSERT INTO DWT04V_ACC_FND.DW_FND_AGG_DAILY_STOCK_FCT (
    business_date,
    loc_wid,
    item_wid,
    location_soh_units,
...
SELECT
    ...
    COALESCE(fct3.lcp_intake, 0) AS lcp_intake,
    COALESCE(fct3.lcp_gross, 0) AS lcp_gross,
  ...
  date '2025-08-12'  AS eff_from_dt, -- Effective from the date of the run.
    ...
    4613 AS run_id, -- Populated from a script variable.
    NULL AS update_run_id, -- NULL for new records.
    'FND2130.300.TARGET.sql' AS job_id, -- Populated from a script variable.
    NULL AS update_job_id -- NULL for new records.
FROM
    DWT04T_TMP_ACC_FND.FND_STK_FCT_PVT_STG AS pvt
LEFT OUTER JOIN
    DWT04T_TMP_ACC_FND.FND_STK_FCT_01_FCT_STG AS fct1
        ON pvt.business_date = fct1.business_date
        AND pvt.loc_wid = fct1.loc_wid
        AND pvt.item_wid = fct1.item_wid
LEFT OUTER JOIN
    DWT04T_TMP_ACC_FND.FND_STK_FCT_02_FCT_STG AS fct2
        ON pvt.business_date = fct2.business_date
        AND pvt.loc_wid = fct2.loc_wid
        AND pvt.item_wid = fct2.item_wid
LEFT OUTER JOIN
    DWT04T_TMP_ACC_FND.FND_STK_FCT_03_FCT_STG AS fct3
        ON pvt.business_date = fct3.business_date
        AND pvt.loc_wid = fct3.loc_wid
        AND pvt.item_wid = fct3.item_wid;
...
--.IF ERRORCODE <> 0 THEN .QUIT 101;
UPDATE DWT04V_ACC_FND.DW_FND_AGG_DAILY_STOCK_FCT
    SET  wkly_flg = 'N'
WHERE
    business_date BETWEEN date '2025-08-10' AND date '2025-08-16'
    AND business_date <> date '2025-08-11' -- Exclude the current run date from the update
    AND wkly_flg = 'Y';
--.IF ERRORCODE <> 0 THEN .QUIT 101;
...
COLLECT STATISTICS COLUMN(business_date,loc_wid,item_wid) ON DWT04T_ACC_FND.DW_FND_AGG_DAILY_STOCK_FCT
;
--.IF ERRORCODE <> 0 THEN .QUIT 101
