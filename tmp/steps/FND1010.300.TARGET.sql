...
# ===============================================================================
# ===============================================================================
# File Name           : FND1010.300.TARGET.sql
# Pattern             : 
...
# -------------------------------------------------------------------------------
# 1.0  | 2025-08-22 |  Mr Primark                |  INITIAL CODE
*/

/* Set query band */
SET QUERY_BAND = 'ApplicationName=TJC;Group=FND_BASE;JobId=FND1010.300.TARGET.sql;JobSeq=300;Instance=T04;RUNID=2245;STREAMID=TEST;' UPDATE FOR SESSION
;
...
WITH
  ...
  DY AS (
    SELECT
      ...
      dy_of_wk_num,
      dy_of_yr_num,
      pmk_prior_cal_dy_wid
    FROM DWT04A_IDW.CSTM_CAL_DY
    WHERE
      cur_flg = 'Y'
  ),
  ...
  WK AS (
    SELECT
      ...
      pmk_wk_of_prd_num,
      pmk_wk_of_yr_num,
      pmk_prior_cal_wk_wid,
      cstm_cal_prd_id,
      cstm_cal_prd_wid
    FROM DWT04A_IDW.CSTM_CAL_WK
    WHERE
      cur_flg = 'Y'
  ),
  ...
  MT AS (
    SELECT
      ...
      cstm_cal_mth_id,
      cstm_cal_wk_id
    FROM DWT04A_IDW.CSTM_CAL_MTH_WK
    WHERE
      cur_flg = 'Y'
  ),
  ...
  PD AS (
    SELECT
      ...
      pmk_prd_end_dt,
      pmk_prd_of_yr_num,
      pmk_prior_cal_prd_wid,
      cstm_cal_prd_id,
      cstm_cal_yr_id
    FROM DWT04A_IDW.CSTM_CAL_MTH
    WHERE
      cur_flg = 'Y'
  ),
  ...
  YR AS (
    SELECT
      ...
      pmk_yr_strt_dt,
      pmk_yr_end_dt,
      pmk_prior_cal_yr_wid,
      cal_type_cd
    FROM DWT04A_IDW.CSTM_CAL_YR
    WHERE
      cur_flg = 'Y'
  ),
...
SELECT
  ...
  -- Surrogate and ID Keys
  TYCD.cstm_cal_dy_wid AS ty_cstm_cal_dy_wid,
  TYWK.cstm_cal_wk_wid AS ty_cstm_cal_wk_wid,
  TYPD.cstm_cal_prd_wid AS ty_cstm_cal_prd_wid,
  TYYR.cstm_cal_yr_wid AS ty_cstm_cal_yr_wid,
  LYCD.cstm_cal_dy_wid AS ly_cstm_cal_dy_wid,
  LYWK.cstm_cal_wk_wid AS ly_cstm_cal_wk_wid,
  LYPD.cstm_cal_prd_wid AS ly_cstm_cal_prd_wid,
  LYYR.cstm_cal_yr_wid AS ly_cstm_cal_yr_wid,
  LLYCD.cstm_cal_dy_wid AS lly_cstm_cal_dy_wid,
  LLYWK.cstm_cal_wk_wid AS lly_cstm_cal_wk_wid,
  LLYPD.cstm_cal_prd_wid AS lly_cstm_cal_prd_wid,
  LLYYR.cstm_cal_yr_wid AS lly_cstm_cal_yr_wid,
  LLLYCD.cstm_cal_dy_wid AS llly_cstm_cal_dy_wid,
  LLLYWK.cstm_cal_wk_wid AS llly_cstm_cal_wk_wid,
  LLLYPD.cstm_cal_prd_wid AS llly_cstm_cal_prd_wid,
  LLLYYR.cstm_cal_yr_wid AS llly_cstm_cal_yr_wid,
  NYCD.cstm_cal_dy_wid AS ny_cstm_cal_dy_wid,
  NYWK.cstm_cal_wk_wid AS ny_cstm_cal_wk_wid,
  NYPD.cstm_cal_prd_wid AS ny_cstm_cal_prd_wid,
  NYYR.cstm_cal_yr_wid AS ny_cstm_cal_yr_wid
FROM
