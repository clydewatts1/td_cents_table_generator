...
# ===============================================================================
# ===============================================================================
# File Name           : FND2102.300.TARGET.sql
# Pattern             : 
...
# -------------------------------------------------------------------------------
# 1.0  | 2025-08-12 |  Mr Primark                |  INITIAL CODE
*/

/* Set query band */
SET QUERY_BAND = 'ApplicationName=TJC;Group=FND_BASE;JobId=FND2102.300.TARGET.sql;JobSeq=300;Instance=T04;RUNID=2507;STREAMID=TEST;' UPDATE FOR SESSION
;
...
DELETE FROM DWT04T_TMP_ACC_FND.FND_STK_FCT_01_FCT_STG
;

--.IF ERRORCODE <> 0 THEN .QUIT 101

INSERT INTO DWT04T_TMP_ACC_FND.FND_STK_FCT_01_FCT_STG
(
    business_date,
    loc_wid,
    item_wid,
...
WITH
    ...
    STPSEL AS (
        SELECT DISTINCT
            ...
            loc_wid,
            pmk_stop_sell
        FROM DWT04A_IDW.PMK_LOC_ITEM AS PLI
        WHERE
            cur_flg = 'Y'
            AND pmk_stop_sell = 'Y'
            AND ssor_id = 4
    ...
    INV_VOL AS (
        SELECT
            ...
            ssor_id,
            tsactn_type_cd
        FROM DWT04A_IDW.INVTRY_TSACTN AS INV
        WHERE
            (
                TRUNC(INV.eff_from_dttm) BETWEEN DATE '2025-08-11' AND DATE '2025-08-11' + 1
                OR TRUNC(INV.eff_to_dttm) BETWEEN DATE '2025-08-11' AND '2025-08-11' + 1
            )
            AND INV.cur_flg = 'Y'
            AND INV.ssor_id <> 3
    ...
    STK_ADJS_TY AS (
        ...
        FROM INV_VOL AS I
        INNER JOIN DWT04A_IDW.INVTRY_TSACTN_ITEM AS ITM
    ...
    STK_ADJ AS (
        SELECT
            ...
            item_wid,
            /* Always use "RUNDATE" */
            DATE '2025-08-11' AS business_date,
            CASE
        ...
        WHERE
            ...
            AND CAST(STK_ADJS_TY.stock_adjustment_date AS DATE) BETWEEN TD_SUNDAY(DATE '2025-08-11') AND TD_SUNDAY(DATE '2025-08-11') + 6
    ),
    STADJ AS (
        SELECT
            STK_ADJ.loc_wid,
    ...
    MKD AS (
        ...
        FROM DWT04A_ACC_ORR.DW_STOCK_MARKDOWN_DAILY_FV AS MKD1
        WHERE
            MKD1.current_flag = 'Y'
            AND MKD1.status = 'L'
            /* Get markdown at point in time */
            AND DATE '2025-08-11' BETWEEN MKD1.item_price_from_date AND MKD1.item_price_to_date
    ...
    PRCT AS (
        ...
        FROM DWT04A_ACC_ORR.DW_ITEM_PRICE_DETAIL_FV AS IPD
        LEFT OUTER JOIN DWT04A_ACC_ORR.DW_PMK_ACCNTG_CAL_DV AS CAL
        ...
        WHERE
            ...
            AND DATE '2025-08-11' BETWEEN IPD.item_price_from_date AND IPD.item_price_to_date
...
WHERE
    STK.stock_inventory_date = DATE '2025-08-11'  /* Use RUNDATE */
...
COLLECT STATISTICS COLUMN(business_date,loc_wid,item_wid) ON DWT04T_TMP_ACC_FND.FND_STK_FCT_01_FCT_STG
;
--.IF ERRORCODE <> 0 THEN .QUIT 101
