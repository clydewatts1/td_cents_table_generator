...
/* Set query band */
SET QUERY_BAND = 'ApplicationName=TJC;Group=FND_BASE;JobId=FND2105.300.TARGET.sql;JobSeq=300;Instance=T04;RUNID=6337;STREAMID=TEST;' UPDATE FOR SESSION
;
    ...
    FROM
        (
            SELECT
                ...
                TSP.item_wid,
                date '2025-08-19' AS business_date,
            ...
            WHERE
                ...
                CAST(TSP.transfer_dt AS DATE) BETWEEN date '2025-08-17'AND date '2025-08-17'+ 6
                AND current_flg = 'Y'
            GROUP BY
                1,
                2,
...
TCV AS ( /* SSBI1066 */
    ...
    WHERE
        ...
        AND ITM.cur_flg = 'Y'
        /* Accumated sum of the week */
        AND WO.pmk_wo_dttm BETWEEN date '2025-08-17'AND date '2025-08-19'
    GROUP BY
        1,
        2,
3
...
STC AS ( /* SSBI1064 */
    ...
    WHERE
        /* Accumated sum of the week */
        STK_CNT_TY.stock_count_dt BETWEEN date '2025-08-17' AND date '2025-08-19'
        AND current_flg = 'Y'
    GROUP BY
        1,
        2,
...
TSFC AS ( /* SSBI1065 */
    ...
    WHERE
        /* Accumated sum of the week */
        STC1.transfer_dt BETWEEN date '2025-08-17' AND date '2025-08-19'
        AND current_flg = 'Y'
    GROUP BY
        1,
        2,
...
/* received stock */
    ...
    WHERE
        /* Accumated sum of the week */
        CAST(TSFRL.receipt_dt AS DATE) BETWEEN date '2025-08-17'AND date '2025-08-19'
    GROUP BY
        1,
        2,
3
...
TSFSHP AS ( /* SSBI1063 - Get the shipping quantity for a transfer */
    ...
    WHERE
        ...
        AND SP_TY.shipment_dt BETWEEN date '2025-08-17'AND date '2025-08-19' /* For This primark Week */
    GROUP BY
        1,
        2,
3
...
STKA AS (
    SELECT
        ...
        loc_wid AS location_wid,
        date '2025-08-19' AS business_date,
        CEILING((date '2025-08-19') - CAST(first_stock_inventory_dt AS DATE)) / 7 AS soh_age_in_weeks
    FROM
...
DSOH AS (
  /* get retail amount from stock pos */
    ...
    WHERE
        stock_inventory_date = date '2025-08-19'
),
FCT AS (
    /* all stock measures
