...
/* Set query band */
SET QUERY_BAND = 'ApplicationName=TJC;Group=FND_BASE;JobId=FND2202.300.TARGET.sql;JobSeq=300;Instance=T04;RUNID=3430;STREAMID=TEST;' UPDATE FOR SESSION
;
...
DELETE FROM DWT04T_TMP_ACC_FND.FND_SLS_FCT_01_FCT_STG
;

--.IF ERRORCODE <> 0 THEN .QUIT 101

INSERT INTO DWT04T_TMP_ACC_FND.FND_SLS_FCT_01_FCT_STG
(
    business_date,
    loc_wid,
    item_wid,
...
WITH TRANSACTIONS_PREPARED AS (
    SELECT
        ...
        nrt_processing_ind,
        retail_type
    FROM DWT04A_ACC_ODS.DW_ODS_SLS_TXN_LN_FV AS SLS
    -- Filter for yesterday's sales data.
    WHERE SLS.sales_business_dt = date '2025-08-10'
...
VOL_SLS_TXN_TY_DATA AS (
    SELECT
        loc_wid,
        item_wid,
        date '2025-08-10' AS business_day_id,
...
VOL_POS_TENDER_TYPE AS (
    SELECT
        ...
        CASE
            ...
            ELSE 'Others'
        END AS tender_type
    FROM DWT04A_ACC_ORR.DW_POS_TENDER_TYPE_DV AS TND
    WHERE current_flg = 'Y'
),
...
VOL_SLS_TSACTN AS (
    SELECT
        ...
        tsactn_type_cd,
        ssor_id
    FROM DWT04A_IDW.SLS_TSACTN
    WHERE
        CAST(rptd_as_dttm AS DATE) = DATE '2025-08-10'
        AND ssor_id <> 3
...
VOL_POS_DVC_EV AS (
    SELECT
        ...
        pos_dvc_ev_dttm,
        ssor_id
    FROM DWT04A_IDW.POS_DVC_EV
    WHERE
        CAST(pos_dvc_ev_dttm AS DATE) = DATE '2025-08-10'
        AND ssor_id <> 3
),
...
VOL_SLS_PMT_LN AS (
    SELECT
        ...
        pmt_appl_dttm,
        ssor_id
    FROM DWT04A_IDW.SLS_PMT_LN
    WHERE
        CAST(pmt_appl_dttm AS DATE) = DATE '2025-08-10'
        AND ssor_id <> 3
),
...
VOL_PMT_TSACTN AS (
    SELECT
        ...
        tndrd_pmt_dttm,
        ssor_id
    FROM DWT04A_IDW.PMT_TSACTN
    WHERE
        CAST(tndrd_pmt_dttm AS DATE) = DATE '2025-08-10'
        AND ssor_id <> 3
),
...
VOL_TENDER_SALES_TXN_TY AS (
    SELECT
        ...
        TNDR_TY.loc_wid,
        DATE '2025-08-10' AS sales_business_date,
        TNDR_TY."effective_from_dt",
        TNDR_TY."tender_type_code",
        TNDR_TY."tender_sales_amount"
    FROM TNDR_TY
    WHERE
        TNDR_TY."sales_business_date" = DATE '2025-08-10'
),
