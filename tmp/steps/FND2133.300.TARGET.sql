...
# -------------------------------------------------------------------------------
# 1.0  | 2025-08-13 |  Mr Primark                |  INITIAL CODE
*/

/* Set query band */
SET QUERY_BAND = 'ApplicationName=TJC;Group=FND_BASE;JobId=FND2133.300.TARGET.sql;JobSeq=300;Instance=T04;RUNID=3406;STREAMID=TEST;' UPDATE FOR SESSION
;
...
-- 4.  Merge data from the daily staging table (FND_STK_FCT_PVT_STG).
...
USING DWT04T_TMP_ACC_FND.FND_STK_FCT_PVT_STG AS src
...
WHEN MATCHED THEN
    UPDATE SET
        ...
        update_run_id = 3406, -- Placeholder for the run ID
        update_job_id = 'FND2133.300.TARGET.sql' -- The name of the job that is running this script.
...
WHEN NOT MATCHED THEN
    ...
    VALUES (
        ...
        date '2025-08-16', -- Teradata function to get the date of the Saturday of the week.
        date '2024-08-13', -- Calculate the date for the same day last year (approximated).
        td_saturday(td_sunday(date '2025-08-16')+1), -- Last years end of business week
        date '2025-08-13', -- The effective start date for this record.
        date '2025-08-16' - 7, -- Last weeks equivalent business date
        DATE '3500-12-31', -- A high-date to indicate the record is currently active.
        0, -- Deletion indicator; 0 means the record is active.
        3406, -- Placeholder for the initial run ID.
        NULL, -- update_run_id is NULL for new records.
        'FND2133.300.TARGET.sql', -- The name of the job that created this record.
...
USING DWT04V_ACC_FND.DW_FND_LOC_ITEM_PVT_FCT AS src
    -- Define the join condition to match records between the source and target tables.
    ON trg.business_date = date '2025-08-12' -- this is todays date
    AND src.business_date  = date '2024-08-13' -- last years date
    AND trg.loc_wid = src.loc_wid
    AND trg.item_wid = src.item_wid
WHEN MATCHED THEN
    UPDATE SET
