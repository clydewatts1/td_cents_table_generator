...
SET
    ty_stock_ind = 0
WHERE
    business_date = date '${EFF_FROM_DT}';
.IF ERRORCODE <> 0 THEN .QUIT 101;
...
USING DW${INSTANCE}T_TMP_ACC_FND.FND_STK_FCT_PVT_STG AS src
    ...
    AND trg.loc_id = src.location_id
    AND trg.item_id = src.item_id
...
WHEN MATCHED THEN
    UPDATE SET
        ...
        update_run_id = ${RUNID}, -- Placeholder for the run ID
        update_job_id = '${JOB}' -- The name of the job that is running this script.
...
WHEN NOT MATCHED THEN
    ...
    VALUES (
        ...
        date '${WEEK_START_DT}', -- Teradata function to get the date of the Saturday of the week.
