/*
# ===============================================================================
#                      PRIMARK
#
# ===============================================================================
# ===============================================================================
# File Name           : OLR1001.100.TRANS.sql
# Pattern             : 
# Purpose             : Insertion in TRANS table
# Author              : TERADATA
# File Type           : SQL
# Creation Date       : 2025-07-24
# -------------------------------------------------------------------------------
# Change History :
# Ver  | Date        |  Modified By           |  Change Description
# -------------------------------------------------------------------------------
# Automatically generated by 10cents_table_generator
# 1.0  | 2025-07-24 | TERADATA               |  INITIAL CODE
# Build Time 08:26:58
# 1.0  | 2025-07-24 | TERADATA               |  INITIAL CODE
*/

BEGIN TRANSACTION
;
.IF ERRORCODE <> 0 THEN .QUIT 101

/* Lock target table */
LOCKING DW${INSTANCE}V_ACC_FND.FND_RECLASFCN_STYL_SBCLASS_XREF FOR WRITE
;
.IF ERRORCODE <> 0 THEN .QUIT 101

/* Update the target table by closing of the previous records */
UPDATE TRG FROM  DW${INSTANCE}V_ACC_FND.FND_RECLASFCN_STYL_SBCLASS_XREF AS TRG
                ,DW${INSTANCE}T_TMP_ACC_FND.FND1040_VALID AS SRC
SET eff_to_dt = SRC.eff_from_dt
  , update_job_id = SRC.job_id
  , update_run_id = SRC.run_id
WHERE TRG.eff_to_dt = '9999-12-31'
AND TRG.styl_wid = SRC.styl_wid 
    
;
.IF ERRORCODE <> 0 THEN .QUIT 101

/* INSERT INTO target table */
INSERT INTO DW${INSTANCE}V_ACC_FND.FND_RECLASFCN_STYL_SBCLASS_XREF
(
    styl_wid, 
    item_sbclas_wid, 
    styl_sbclass_vers_num, 
    /* Control Columns */
    eff_from_dt,
    eff_to_dt,
    del_ind,
    run_id,
    update_run_id,
    job_id,
    update_job_id
)

SELECT
    SRC.styl_wid AS styl_wid, 
    SRC.item_sbclas_wid AS item_sbclas_wid, 
    SRC.styl_sbclass_vers_num AS styl_sbclass_vers_num, 
    SRC.eff_from_dt,
    SRC.eff_to_dt,
    SRC.del_ind,
    SRC.run_id,
    SRC.update_run_id,
    SRC.job_id,
    SRC.update_job_id
FROM DW${INSTANCE}T_TMP_ACC_FND.FND1040_VALID AS SRC
;
.IF ERRORCODE <> 0 THEN .QUIT 101

/* End of INSERT statement */


END TRANSACTION
.IF ERRORCODE <> 0 THEN .QUIT 101