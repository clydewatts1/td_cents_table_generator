...
WITH
  ...
  DY AS (
    ...
    FROM DW${INSTANCE}A_IDW.CSTM_CAL_DY
    WHERE
      cur_flg = 'Y'
  ),
  ...
  WK AS (
    SELECT
      ...
      cstm_cal_prd_id,
      cstm_cal_prd_wid
    FROM DW${INSTANCE}A_IDW.CSTM_CAL_WK
    WHERE
      cur_flg = 'Y'
  ),
  ...
  MT AS (
    SELECT
      ...
      cstm_cal_mth_id,
      cstm_cal_wk_id
    FROM DW${INSTANCE}A_IDW.CSTM_CAL_MTH_WK
    WHERE
      cur_flg = 'Y'
  ),
  ...
  PD AS (
    SELECT
      ...
      cstm_cal_prd_id,
      cstm_cal_yr_id
    FROM DW${INSTANCE}A_IDW.CSTM_CAL_MTH
    WHERE
      cur_flg = 'Y'
  ),
...
COLLECT STATISTICS INDEX(calendar_dt) ON DW${INSTANCE}T_TMP_ACC_FND.FND1010_DW_FND_DATE_DIM
;
.IF ERRORCODE <> 0 THEN .QUIT 101
